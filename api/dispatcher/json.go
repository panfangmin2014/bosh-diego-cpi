...
import (
	...
	bdcapi "github.com/cppforlife/bosh-diego-cpi/api"
)
...
type JSON struct {
	actionFactory bdcaction.Factory
	caller        Caller
...
func (c JSON) buildCloudError(err error) []byte {
	...
	if typedErr, ok := err.(bdcapi.CloudError); ok {
	...
	if typedErr, ok := err.(bdcapi.RetryableError); ok {
